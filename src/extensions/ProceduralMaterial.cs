...
namespace RayTracer
{
    ...
    public class ProceduralMaterial : Material
    {
        ...
        {
            double scaledU = coord.U * scaleU;
            double scaledV = coord.V * scaleV;

            switch (pattern)
            {
                case PatternType.Checkers:
                    return GenerateCheckersPattern(scaledU, scaledV);
                case PatternType.Stripes:
                    return GenerateStripesPattern(scaledU, scaledV);
                default:
                    return color1;
            }
        }

        private Color GenerateCheckersPattern(double u, double v)
        {
            int uSquare = (int)Math.Floor(u);
            int vSquare = (int)Math.Floor(v);
            
            bool useColor1 = (uSquare + vSquare) % 2 == 0;
            return useColor1 ? color1 : color2;
        }

        private Color GenerateStripesPattern(double u, double v)
        {
            int stripe = (int)Math.Floor(u);
            
            bool useColor1 = stripe % 2 == 0;
            return useColor1 ? color1 : color2;
        }
    }
}
